...
func scanEstimulo(row rowScanner) (*domain.Estimulo, error) {
	e := &domain.Estimulo{}
	var fechaIdeal, createdAt string
	var fechaAplicado sql.NullString
	if err := row.Scan(&e.ID, &e.EmpleadoID, &e.Tipo, &e.Contenido, &e.Intensidad, &e.Canal, &e.Estado, &fechaIdeal, &fechaAplicado, &e.OrigenRecomendacion, &createdAt); err != nil {
		return nil, err
	}
...
const estimuloColumns = `SELECT id, empleado_id, tipo, contenido, intensidad, canal, estado, fecha_ideal, fecha_aplicado, origen_recomendacion, created_at FROM estimulos`

func listEstimulos(ctx context.Context, db queryer, query string, args ...any) ([]domain.Estimulo, error) {
...
func (s *Store) GetEstimulo(ctx context.Context, id int64) (*domain.Estimulo, error) {
	e, err := scanEstimulo(s.db.QueryRowContext(ctx,
...
func (s *Store) ListEstimulosPorEmpleado(ctx context.Context, empleadoID int64) ([]domain.Estimulo, error) {
	return listEstimulos(ctx, s.db,
		estimuloColumns+` WHERE empleado_id = ? ORDER BY created_at DESC`, empleadoID)
}

// ListEstimulos returns stimuli using the same state-specific ordering exposed
// by the service before persistence was moved behind the repository port.
func (s *Store) ListEstimulos(ctx context.Context, estado string) ([]domain.Estimulo, error) {
	switch estado {
	case "pendiente":
		return listEstimulos(ctx, s.db,
			estimuloColumns+` WHERE estado = 'pendiente' ORDER BY fecha_ideal`)
	case "aplicado":
		return listEstimulos(ctx, s.db,
			estimuloColumns+` WHERE estado = 'aplicado' ORDER BY created_at DESC`)
	default:
		return listEstimulos(ctx, s.db, estimuloColumns+` ORDER BY created_at DESC`)
	}
}

func (s *Store) ListEstimulosPendientes(ctx context.Context) ([]domain.Estimulo, error) {
	return s.ListEstimulos(ctx, "pendiente")
}

// transitionEstimulo marks a stimulus as applied conditionally inside the
// transaction. Only pending stimuli transition.
func transitionEstimulo(ctx context.Context, db execer, id int64, at time.Time) (bool, error) {
